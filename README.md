...
## Почему это не просто готовая API-модель

QalaVision AI честно использует OpenAI API как слой анализа изображения и текста. Команда не заявляет, что обучила нейросеть с нуля, и не заявляет наличие собственной YOLO/CNN-модели.

Но OpenAI API не используется “как есть”. В проект добавлен собственный слой доменной адаптации QalaVision AI:

- фиксированная таксономия городских дефектов;
- baseline-классификатор по ключевым словам без внешних ML-библиотек;
- нормализация ответа OpenAI к городским классам;
- priority engine для расчёта риска;
- cost estimator для ориентировочной стоимости ремонта;
- SLA logic для рекомендуемого срока реагирования;
- human-in-the-loop флаг для заявок, требующих проверки оператором;
- dashboard и risk map для операционного использования.

Такой подход делает MVP не wrapper над OpenAI, а доменную систему поддержки решений для городской инфраструктуры.

## Где здесь искусственный интеллект

В MVP используется гибридный AI-подход: готовая vision/text модель для первичного анализа фото и текста + собственный слой доменной адаптации QalaVision AI. Этот слой приводит результат модели к фиксированным городским классам, проверяет его baseline-классификатором, считает риск, стоимость, срок и необходимость проверки оператором.
...
- нормализует результат через `lib/domain-adapter.ts`;
- сверяет класс с baseline-классификатором `lib/baseline-classifier.ts`;
- считает приоритет через `lib/priority-engine.ts`;
- оценивает стоимость через `lib/cost-estimator.ts`;
- применяет SLA и human-in-the-loop правила;
...
Методология MVP основана на объяснимой оценке риска. OpenAI выполняет первичный анализ изображения и текста, но итоговое решение проходит через собственную таксономию, baseline-классификатор, priority engine и cost estimator.
...
| Дубликаты | Будущий слой | Поиск похожих жалоб в районе |
...
Проверить собственный baseline-классификатор:
...
lib/
  baseline-classifier.ts      Собственный keyword baseline без внешних ML-библиотек
  cost-estimator.ts           Оценка стоимости ремонта
  defect-taxonomy.ts          Таксономия городских дефектов и SLA
  domain-adapter.ts           Нормализация raw OpenAI ответа
  priority-engine.ts          Расчёт urgency, risk level и SLA
  ...
  utils.ts                    UI helper
  validation-examples.ts      Примеры для ручной проверки baseline

scripts/
  validate-baseline.ts        CLI-проверка baseline-классификатора
